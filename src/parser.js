...
export default class Parser {
        ...
        case tokens.comment:
          if (lastAdded) {
            if (
              ...
              (next && next[TOKEN.TYPE] === tokens.space) ||
              lastAdded === "insensitive"
            ) {
              const lastComment = getProp(node, "spaces", lastAdded, "after") || "";
              const rawLastComment =
                getProp(node, "raws", "spaces", lastAdded, "after") || lastComment;
  ...
  convertWhitespaceNodesToSpace(nodes, requiredSpace = false) {
    ...
    nodes.forEach((n) => {
      ...
      space +=
        spaceBefore + this.lossySpace(n.spaces.after, requiredSpace && spaceBefore.length === 0);
      rawSpace +=
        spaceBefore +
        n.value +
        this.lossySpace(n.rawSpaceAfter, requiredSpace && rawSpaceBefore.length === 0);
    });
    if (rawSpace === space) {
      rawSpace = undefined;
    }
  ...
  isNamedCombinator(position = this.position) {
    return (
      ...
      this.tokens[position + 0][TOKEN.TYPE] === tokens.slash &&
      this.tokens[position + 1] &&
      this.tokens[position + 1][TOKEN.TYPE] === tokens.word &&
      this.tokens[position + 2] &&
      this.tokens[position + 2][TOKEN.TYPE] === tokens.slash
    );
  }
  namedCombinator() {
  ...
  combinator() {
    ...
    if (
      ...
      this.tokens[nextSigTokenPos][TOKEN.TYPE] === tokens.closeParenthesis
    ) {
    ...
    if (node) {
      if (spaceOrDescendantSelectorNodes) {
        let { space, rawSpace } = this.convertWhitespaceNodesToSpace(
  ...
  unexpected() {
    return this.error(
      ...
      this.currToken[TOKEN.START_POS],
    );
  }
  ...
  pseudo() {
    ...
    if (this.currToken[TOKEN.TYPE] === tokens.word) {
      this.splitWord(false, (first, length) => {
        ...
        if (length > 1 && this.nextToken && this.nextToken[TOKEN.TYPE] === tokens.openParenthesis) {
